...
const downloadOrderTicket = async (order: SalesOrder, attendedByName?: string) => {
  ...
  center("Orden de Venta #" + seqTextRender, styles.emphasis.font, styles.emphasis.color, styles.emphasis.lineHeight);
  ...
  muted("P/U: " + formatBs(it.unitPrice) + "  ×  " + it.quantity);
    ...
    divider();
  center(cfg.order.farewell, styles.subtitle.font, styles.subtitle.color, styles.subtitle.lineHeight + 6);
  ...
  const usedHeightPx = Math.ceil(usedHeight * scale);
    ...
    a.href = url;
  a.download = "orden-venta-" + (order.sequence ?? order.id) + ".png";
    a.rel = "noopener";
    ...
    console.warn("No se pudo generar el ticket de la orden de venta:", err);
    return false;
  }
};
...
export default function SalesOrdersPage() {
  ...
  const handleSubmit = async () => {
    ...
    // Crear orden en Supabase (no descuenta stock)
    ...
    // Mapear a SalesOrder para ticket
